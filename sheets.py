...
def get_keywords():
    ...
    for row in records:
        ...
        try:
            limit = int(limit_raw)
        except (ValueError, TypeError):          # ✅ FIX: indent ตรงกับ try
            limit = 100
    ...
    return result  # ✅ FIX: ย้ายออกนอก for loop
...
def get_existing_links():
    ...
    for row in records:
        ...
        if link:
            existing.add(link)
    return existing  # ✅ FIX: ย้ายออกนอก for loop
...
def append_unique_posts(new_rows):
    ...
    else:                                        # ✅ FIX: indent ตรงกับ if new_rows
